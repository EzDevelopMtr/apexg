...
/**
 * The APEX GYM lockup: a hollow peak with a lime summit, over the wordmark.
 *
 * Drawn rather than shipped as an image so it stays sharp at any size and
 * inherits the accent token — a change of brand colour moves it too.
 */
    ...
    <div className={`flex flex-col items-center ${className}`}>
      ...
      >
        <path
          ...
          className="text-body"
        />
        <path d="M60 54 L80 90 L40 90 Z" className="fill-brand" />
      </svg>
      ...
      {!markOnly && (
        <>
          ...
          <span className="mt-1.5 flex items-center gap-2">
            <span className={`h-px bg-brand ${RULE[size]}`} />
            <span
