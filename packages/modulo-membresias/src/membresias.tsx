...
export default function Membresias({ activeSection, rol = "administrador" }: MembresiasProps) {
  const puedeGestionar = puedeGestionarMembresias(rol);
  ...
  return (
    <div className="p-8"><div className="mx-auto max-w-7xl">
      <div className="mb-8 flex flex-wrap items-end justify-between gap-4"><div><h1 className="text-3xl font-bold text-slate-900">{tituloVista}</h1><p className="mt-2 text-slate-500">Administra los valores y condiciones de las membresías.</p></div>{puedeGestionar && <Button onClick={abrirNuevo}><Plus size={18} />Nuevo plan</Button>}</div>
      {puedeGestionar && (creando || editando) && <Modal abierto={creando || editando !== null} titulo={editando ? "Editar plan" : "Nuevo plan"} onCerrar={() => { setEditando(null); setCreando(false); setFormulario(formularioVacio()); }}><form onSubmit={guardar} className="space-y-5">
