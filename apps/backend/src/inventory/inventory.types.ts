 ...
 * de medida, existencias y mínimo — y NO tiene columnas para SKU,
 * categoría, precio de costo/venta ni proveedor: campos que sí existen en
 * el modelo de dominio ya construido en el frontend
 * (`packages/core/src/domain/inventory.ts`). Ese desfase es del alcance
 * del ERS, no un olvido aquí — se deja para `database-architect` decidir
 * si esas columnas se agregan cuando el cliente afine el alcance.
 */
